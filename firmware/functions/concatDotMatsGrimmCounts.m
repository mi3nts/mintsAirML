function grimmCountsDataAll = concatDotMatsGrimmCounts(dotMatsFolder,startDate,endDate)
%GETALLGRIMMDATA 
...
%% Load the files you need as one 
    ...
    for n =1:length(duration)   
        fileName = dotMatsFolder+ "/Spectrometor_"+ num2str(year(duration(n)),'%04.f')+"-" +num2str(month(duration(n)),'%02.f')+"-"+num2str(day(duration(n)),'%02.f')+"-C.mat"  

        if (exist(fileName , 'file') == 2)
                load(fileName) 
                grimmCountsDataAll = [grimmCountsDataAll; grimmCountsData];
        end
