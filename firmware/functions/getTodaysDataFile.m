...
%% Create Dot Mat Files  
    ...
    for n =1 :length(mintsDataFiles)
        ...
        if sensorName ~= "SKYCAM"
            mintsRetimed = retime(rmmissing( table2timetable(mintsData)),'regular',@nanmean,'TimeStep',dtIn);
            if(n==1)  
                 mints = mintsRetimed;   
                else
                 mints =  rmmissing(synchronize(mints,mintsRetimed,'intersection'));
            end
        end
