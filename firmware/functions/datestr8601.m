...
%  str = datestr8601(dtx,tkn1,tkn2,tkn3,...)
%  [str1,str2,str3,...] = datestr8601(dtx,tkn1,tkn2,tkn3,...)
%  [...] = datestr8601([],...)
%
...
assert(rwf(size(dtv)),'First input <dtx> must be a date vector, serial date number, or datetime.')
...
for m = 1:numel(varargin)
	...
	tkn = varargin{m};
	assert(ischar(tkn)&&rwf(size(tkn)),'%s input must be a token (1xN character).',sfx)
	tkl = numel(tkn);
	iso = strcmp(tkn(tkl),'*');
...
% ISO 8601 timestamp
%
er1 = '%s input (token) is not recognized: ''%s''. Check the help!';
er2 = '%s input (token) contains a trailing date-time separator: ''%s''';
er3 = '%s input (token) is out of sequence or is missing units: ''%s''';
...
assert(~isempty(tkr),er1,sfx,tkn)
assert(numel(tkr)==4,'It appears that you are using Octave with buggy REGEXP.')
assert(isempty(tkr{2})||~isempty(tkr{3}),er2,sfx,tkn)
% Identify timestamp:
stf = strfind({'ymdHMS','ynHMS','YWDHMS'},[tkr{1},tkr{3}]);
ist = find(~[isempty(stf{1}),isempty(stf{2}),isempty(stf{3})]);
assert(~isempty(ist),er3,sfx,tkn)
isw = ist(1)==3;
%
idb = stf{ist(1)};
